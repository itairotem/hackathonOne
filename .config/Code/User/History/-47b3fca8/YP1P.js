...
function drop(event) {
    ...
    // Get the position where the note is dropped
    var targetIndex = -1;
    var mouseX = event.clientX;
    var mouseY = event.clientY;

    // Loop through each note to find the index where the note is dropped
    for (var i = 0; i < notes.length; i++) {
        var noteElement = document.getElementById('note_' + i);
    ...
    // If the dragged element is not dropped on another note, exit the function
    if (targetIndex === -1) {
        return;
    }
