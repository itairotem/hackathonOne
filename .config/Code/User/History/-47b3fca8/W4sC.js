...
// Variable to store the index of the note being dragged
var dragIndex = null;
...
function drag(event) {
    dragIndex = parseInt(event.target.dataset.index);
}
...
function drop(event) {
    event.preventDefault();
    var dropIndex = parseInt(event.target.dataset.index);

    // Swap notes in the array
    var temp = notes[dragIndex];
    notes[dragIndex] = notes[dropIndex];
    notes[dropIndex] = temp;

    renderNotes(); // Update the UI after swapping
}
