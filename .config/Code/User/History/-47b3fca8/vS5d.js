...
function renderNotes() {
    ...
    notes.forEach(function(note, index) {
        ...
        var countdownDate = new Date(note.date).getTime(); // Convert note date to milliseconds
        var now = new Date().getTime(); // Get current time in milliseconds
        var distance = countdownDate - now; // Calculate the difference
        var days = Math.floor(distance / (1000 * 60 * 60 * 24)); // Convert milliseconds to days
        countdown.textContent = 'Days left: ' + days; // Display remaining days
